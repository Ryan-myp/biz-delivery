...
# 导入 biz-delivery 共享模块
# 注意：通过 sys.path 添加 biz-delivery root 来支持相对/绝对导入
_BD_ROOT = str(Path(__file__).parent.parent)
if _BD_ROOT not in __import__("sys").path:
    __import__("sys").path.insert(0, _BD_ROOT)

from scripts.smart_routing import extract_intent, get_scope_weights
from scripts.smart_routing import INTENT_TO_QUERY_TYPE
