...
class TDEngine(EngineBase):
    ...
    def _build_td_prompt(self, filtered: dict, ir: IRDocument, prd_text: str, review_report: Optional[str] = None, cache_dir: str = None) -> str:
        ...
        prompt_parts.append("")
        prompt_parts.append("### 4.3 核心流程图（sequence diagram）")
        prompt_parts.append("```mermaid")
        prompt_parts.append("sequenceDiagram")
        prompt_parts.append("    participant Client")
        prompt_parts.append("    participant API")
        prompt_parts.append("    participant Service")
        prompt_parts.append("    participant DB")
        prompt_parts.append("    Client->>API: 请求")
        prompt_parts.append("    API->>Service: 调用")
        prompt_parts.append("    Service->>DB: 查询/写入")
        prompt_parts.append("    DB-->>Service: 结果")
        prompt_parts.append("    Service-->>API: 响应")
        prompt_parts.append("    API-->>Client: 返回")
        prompt_parts.append("```")
        prompt_parts.append("")
        prompt_parts.append("### 4.4 架构图（architecture diagram）")
        prompt_parts.append("```mermaid")
        prompt_parts.append("graph TB")
        prompt_parts.append("    subgraph 前端层")
        prompt_parts.append("        Web[Web App]")
        prompt_parts.append("        Mobile[Mobile App]")
        prompt_parts.append("    end")
        prompt_parts.append("    subgraph 网关层")
        prompt_parts.append("        Gateway[API Gateway]")
        prompt_parts.append("        Auth[Auth Middleware]")
        prompt_parts.append("    end")
        prompt_parts.append("    subgraph 业务层")
        prompt_parts.append("        Handler[Handler Layer]")
        prompt_parts.append("        Service[Service Layer]")
        prompt_parts.append("    end")
        prompt_parts.append("    subgraph 数据层")
        prompt_parts.append("        DAO[DAO Layer]")
        prompt_parts.append("        Redis[(Redis)]")
        prompt_parts.append("        MySQL[(MySQL)]")
        prompt_parts.append("    end")
        prompt_parts.append("    subgraph 外部服务")
        prompt_parts.append("        Ext[External APIs]")
        prompt_parts.append("    end")
        prompt_parts.append("    Web --> Gateway")
        prompt_parts.append("    Mobile --> Gateway")
        prompt_parts.append("    Gateway --> Auth")
        prompt_parts.append("    Auth --> Handler")
        prompt_parts.append("    Handler --> Service")
        prompt_parts.append("    Service --> DAO")
        prompt_parts.append("    Service --> Redis")
        prompt_parts.append("    Service --> MySQL")
        prompt_parts.append("    Service --> Ext")
        prompt_parts.append("```")
        prompt_parts.append("")
        prompt_parts.append("### 4.5 数据模型图（ER diagram）")
        prompt_parts.append("```mermaid")
        prompt_parts.append("erDiagram")
        prompt_parts.append("    ENTITY ||--o{ RELATED : \"has many\"")
        prompt_parts.append("    ENTITY {")
        prompt_parts.append("        bigint id PK")
        prompt_parts.append("        varchar name")
        prompt_parts.append("        tinyint status")
        prompt_parts.append("        timestamp created_at")
        prompt_parts.append("    }")
        prompt_parts.append("```")
        prompt_parts.append("(根据实际表结构生成)")
        prompt_parts.append("")
        prompt_parts.append("### 4.6 部署架构图（deployment diagram）")
        prompt_parts.append("```mermaid")
        prompt_parts.append("graph LR")
        prompt_parts.append("    LB[Load Balancer] --> Server1[App Server 1]")
        prompt_parts.append("    LB --> Server2[App Server 2]")
        prompt_parts.append("    Server1 --> Redis[(Redis Cluster)]")
        prompt_parts.append("    Server1 --> MySQL1[(MySQL Master)]")
        prompt_parts.append("    Server2 --> MySQL1")
        prompt_parts.append("    Server1 --> MySQL2[(MySQL Slave)]")
        prompt_parts.append("    Server2 --> MySQL2")
        prompt_parts.append("    Server1 --> MQ[(Message Queue)]")
        prompt_parts.append("    Server2 --> MQ")
        prompt_parts.append("```")
        prompt_parts.append("(根据实际部署拓扑调整)")
        prompt_parts.append("")
