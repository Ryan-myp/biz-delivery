...
class TestEngine:
    ...
    def _build_test_prompt(self, filtered: dict, ir: IRDocument, prd_text: str, td_text: Optional[str] = None, cache_dir: str = None) -> str:
        ...
        # 新增：Mock 策略
        prompt_parts.append("### 6. Mock 策略")
        prompt_parts.append("- **DAO Mock**: 使用 gomock/mockery 生成 DAO 接口 mock，避免真实 DB 操作")
        prompt_parts.append("- **Service Mock**: 对外部 Service 调用使用 mock，隔离测试单元")
        prompt_parts.append("- **HTTP Mock**: 使用 httptest.NewRecorder() 测试 HTTP handler")
        prompt_parts.append("- **RPC Mock**: 对外部 RPC 调用使用 mock，模拟不同响应（正常/超时/错误）")
        prompt_parts.append("- **Config Mock**: 使用测试配置覆盖 Apollo/本地配置")
        prompt_parts.append("- **Redis Mock**: 使用 fake-redis 或内存实现")
        prompt_parts.append("")
        
        # 新增：数据准备策略
        prompt_parts.append("### 7. 数据准备策略")
        ...
        prompt_parts.append("    err := CreateAdGroup(req)")
        prompt_parts.append("    req := &CreateAdGroupRequest{Name: \"test-group\", Budget: 1000}")
        prompt_parts.append("func TestCreateAdGroup(t *testing.T) {")
        prompt_parts.append("```go")
        prompt_parts.append("### 5.1 Go 单元测试")
        prompt_parts.append("")
        prompt_parts.append("## 5. 自动化测试代码（可直接运行）")
        ...
        prompt_parts.append("")
        prompt_parts.append("### 5.4 自动化测试代码生成（Go + testify）")
        ...
        prompt_parts.append("```")
        prompt_parts.append("")
        prompt_parts.append("### 5.5 自动化测试代码生成（pytest）")
